...
if (isset($_POST['simpan_serte'])) {
    ...
    mysqli_query($koneksi, "CREATE TABLE IF NOT EXISTS serah_terima (id_serte INT AUTO_INCREMENT PRIMARY KEY, id_aset INT, penerima VARCHAR(255), penyetuju VARCHAR(255), tgl_serte DATE, keterangan TEXT)");
...
if($q_aset && mysqli_num_rows($q_aset) > 0) {
    while($a = mysqli_fetch_assoc($q_aset)) {
        ...
        $aset_opt .= "<option value='{$a['id_aset']}' data-foto='{$fotoClass}'>{$a['kode_aset']} - {$a['nama_aset']}</option>";
    }
} else {
...
$btnBatal  = buttonhref("index.php?pg=kserte&fl=list", "Batal", "light border text-muted px-4", "", "");
$btnSimpan = button("simpan_serte", "Simpan Serah Terima", "primary px-4 shadow", "save", "style='background-color: var(--accent); border:none;'");
...
<form action="" method="POST">
    <div class="d-flex flex-column gap-4">
        ...
        <div>
            <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Pilih Aset</label>
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Pihak Penerima</label>
                <div class="input-group border rounded-3 overflow-hidden shadow-sm bg-light">
                    <span class="input-group-text bg-transparent border-0 text-muted ps-3"><i data-lucide="user" style="width:18px"></i></span>
                    <input type="text" name="penerima" list="listPegawai" class="form-control bg-transparent border-0 shadow-none py-2" placeholder="Ketik atau pilih nama..." required autocomplete="off">
                </div>
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Tanggal Serah Terima</label>
                <input type="date" name="tgl_serte" class="form-control bg-light border rounded-3 py-2 shadow-sm" required value="$tgl_default">
            </div>
        </div>

        <div>
            <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Keterangan</label>
            <textarea name="keterangan" class="form-control bg-light border rounded-3 shadow-sm p-3" rows="3" placeholder="Informasi tambahan..."></textarea>
        </div>
...
echo '  <div class="col-12 col-lg-8">';
            PageHeader("Tambah Serah Terima Aset", "Daftarkan serah terima aset ke dalam sistem");
